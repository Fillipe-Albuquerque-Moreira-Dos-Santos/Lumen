...
Este projeto tem o **Lumen** instalado — um único agente que **documenta o legado** e depois **constrói** features novas ancoradas na verdade extraída.
...
1. **Documentar** — lê o código legado e gera specs em \`_lumen_docs/\` (🟢 confirmado / 🟡 inferido / 🔴 lacuna). Nunca toca no seu código.
2. **Construir** — ao terminar de documentar, o Lumen sugere codar uma feature: \`lumen-ground → lumen-prd → lumen-techspec → lumen-tasks → lumen-build → lumen-review\`.
3. **Verificar** — depois do build, \`lumen-verify\` confere que nenhuma regra 🟢 do legado regrediu.

## Regra absoluta

Documentar e Verificar nunca escrevem código do projeto — só \`.lumen/\` e \`_lumen_docs/\`. Apenas o \`lumen-build\` escreve código, e só dentro do escopo das tasks aprovadas.
`;
}
...
export class Writer {
  ...
  updateGitignore(outputFolder) {
    ...
    const block = ['', '# Lumen', '.lumen/', `${outputFolder}/`, '_lumen/'].join('\n');
