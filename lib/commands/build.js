...
import { readTasks, listBlocks, hasBlocks, resolveBlock, dependencyClosure, hideExcept, restoreHeld } from '../utils/plan.js';
...
// ── Escolha interativa no terminal (listas reais + opção de digitar a sua) ──
const OTHER = '✎ Outra — eu digito a versão exata';
async function choose(inquirer, message, options, opts = {}) {
  const otherLabel = opts.otherLabel || OTHER;
  const customMessage = opts.customMessage || '  Digite exatamente (ex.: Angular 18.2.1):';
  if (!options || !options.length) {
    const { v } = await inquirer.prompt([{ type: 'input', name: 'v', prefix: '', message }]);
    return v.trim();
  }
  const { pick } = await inquirer.prompt([{
    type: 'list', name: 'pick', prefix: '', message, choices: [...options, otherLabel], loop: false, pageSize: 14,
  }]);
  if (pick !== otherLabel) return pick;
  const { custom } = await inquirer.prompt([{
    type: 'input', name: 'custom', prefix: '', message: customMessage,
...
// Arquiteturas sugeridas — o usuário escolhe uma OU digita a sua.
const ARCHITECTURES = [
  'Camadas — Controller → Service → Repository (simples, familiar) ⭐',
  'Clean Architecture — domínio no centro, infra na borda',
  'Hexagonal — Ports & Adapters (domínio isolado da infra)',
  'Vertical Slices — uma fatia por funcionalidade',
  'MVC — Model · View · Controller',
  'Microsserviços — serviços independentes',
  'Event-driven / CQRS — eventos e leitura/escrita separadas',
];

// Pergunta a arquitetura: lista sugerida + "✎ Outra — eu digito". Devolve a escolha.
async function pickArchitecture(inquirer, chalk) {
  const arch = await choose(inquirer, 'Arquitetura do sistema:', ARCHITECTURES, {
    otherLabel: '✎ Outra — eu digito a arquitetura',
    customMessage: '  Digite a arquitetura que quer usar (ex.: Clean + CQRS):',
  });
  console.log(chalk.gray(`\n  Arquitetura escolhida: ${arch}\n`));
  return arch;
}
...
export default async function build(args) {
  ...
  // Extrai --bloco/--block (e a forma --bloco=valor) antes de detectar a feature,
  // para o nome do bloco não ser confundido com o nome da feature.
  let bloco = null;
  const rest = [];
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === '--bloco' || a === '--block') { bloco = (args[++i] ?? '').trim(); continue; }
    const eq = a.match(/^--(?:bloco|block)=(.*)$/);
    if (eq) { bloco = eq[1].trim(); continue; }
    rest.push(a);
  }
  const feature = rest.find(a => !a.startsWith('-')) || null;

  // ── Caminho direto: executar tarefas já geradas de uma feature ──
  if (feature) {
    const featureDir = join(projectRoot, '_lumen', feature);
    restoreHeld(featureDir); // rede de segurança: desfaz ocultação de um run anterior interrompido
    const link = linkFeature(projectRoot, feature);
    if (!link.ok) { console.error(chalk.red(`\n  ${link.reason}.\n`)); process.exit(1); }
    const passthrough = rest.filter(a => a !== feature).map(a => a === '--bg' ? '--detach' : a);
    if (!passthrough.includes('--ide')) passthrough.push('--ide', 'claude');
    const bg = passthrough.includes('--detach');

    // ── Build por bloco: roda só o bloco-alvo (+ dependências), escondendo o resto ──
    if (bloco !== null) {
      if (bg) {
        console.error(chalk.yellow('\n  --bloco ainda não roda em background (--bg). Rode o bloco em primeiro plano.\n'));
        process.exit(1);
      }
      const tasks = readTasks(featureDir);
      if (!tasks.length) {
        console.error(chalk.red(`\n  _lumen/${feature} não tem tasks — gere com o lumen-tarefas primeiro.\n`));
        process.exit(1);
      }
      const blocks = listBlocks(tasks);
      if (!hasBlocks(tasks)) {
        console.error(chalk.yellow(`\n  A feature "${feature}" não tem blocos (campo \`bloco:\` nas tasks).`));
        console.error(chalk.gray('  Rode o lumen-tarefas para gerar os blocos, ou construa tudo: ') + chalk.white(`lumen build ${feature}`) + '\n');
        process.exit(1);
      }
      const target = bloco ? resolveBlock(blocks, bloco) : null;
      if (!target) {
        console.error(chalk.yellow(`\n  Bloco ${bloco ? `"${bloco}" não encontrado` : 'não informado'}. Blocos de "${feature}":\n`));
        for (const b of blocks) console.error(`    ${chalk.hex('#f5c518')('•')} ${chalk.white(b.name)} ${chalk.gray(`(${b.done}/${b.total} prontas)`)}`);
        console.error(chalk.gray(`\n  Ex.: lumen build ${feature} --bloco "${blocks[0].name}"\n`));
        process.exit(bloco ? 1 : 0);
      }
      const keep = dependencyClosure(tasks, target.taskIds);
      const extraDeps = [...keep].filter(id => !target.taskIds.includes(id) && tasks.find(t => t.id === id)?.status !== 'done');
      const hidden = hideExcept(featureDir, keep);
      console.log(chalk.hex('#f5c518')(`\n  Lumen: construindo o bloco "${target.name}" de "${feature}"...`));
      console.log(chalk.gray(`  ${target.taskIds.length} task(s) do bloco${extraDeps.length ? ` + ${extraDeps.length} dependência(s) pendente(s)` : ''} · ${hidden} task(s) de outros blocos em espera\n`));
      let res;
      try {
        res = engineRun(['tasks', 'run', feature, ...passthrough]);
      } finally {
        restoreHeld(featureDir); // sempre revela de volta as tasks escondidas
      }
  ...
  let archLine = 'ARQUITETURA: espelhe a arquitetura e os padrões que o sistema já usa (do _lumen_docs/).';
  let archSummary = 'manter a arquitetura atual';
  if (intent === 'pick') {
    ...
    const arch = await pickArchitecture(inquirer, chalk);
    archLine = `ARQUITETURA: use EXATAMENTE este padrão escolhido pelo usuário (NÃO reabra, NÃO troque): ${arch}.`;
    archSummary = arch;
  }
  ...
  // 5. Kickoff com o stack TRAVADO e o DESTINO definido (um agente interativo)
  ...
  const kickoff =
    `${scope}, em modo automático${grounded}. STACK: ${stackLine} ${archLine} ${dest.line} `
    + 'Pergunte o NOME do sistema: ofereça 2–3 sugestões fiéis ao domínio E SEMPRE inclua, como última opção, "✎ Outra — eu digito o nome" aceitando texto livre (NUNCA decida o nome sozinho; use exatamente o que o usuário digitar). NÃO pergunte stack/versão/arquitetura de novo — já está definido. '
    ...
    + 'Depois gere as tarefas de TODAS as funcionalidades a partir de _lumen_docs/<unit>/ e construa VÁRIAS AO MESMO TEMPO, NESTA MESMA sessão — usando seus próprios subagentes internos e a execução concorrente do motor (NÃO abra agentes/processos separados; é tudo este mesmo agente). Execute até o fim, revise (auditor) e verifique. '
