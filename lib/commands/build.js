...
export default async function build(args) {
  ...
  if (ensureInstalled(projectRoot)) console.log(chalk.gray('  Agentes Lumen instalados neste projeto.'));
  const didSetup = ensureEngineSetup(projectRoot);
  console.log(chalk.gray(didSetup ? '  Motor de execução preparado (skills do motor instaladas). ✓' : '  Motor de execução pronto. ✓'));
  ...
  const kickoff =
    `${scope}, em modo automático${grounded}. STACK: ${stackLine} `
    + 'Pergunte o NOME (sugira opções + deixe o usuário digitar). NÃO pergunte stack/versão de novo — já está definido. '
