...
export default async function build(args) {
  ...
  // 4b. SISTEMA INTEIRO + agente headless (claude/codex) → SEMPRE vários agentes em
  //     paralelo, sem perguntar nada de execução: o Lumen só dispara. (Agente sem
  //     headless cai no caminho de um agente interativo, mais abaixo.)
  if (what === 'all' && supportsHeadless(agentCmd)) {
    ...
    const jobs = streams.length; // todos de uma vez — sem perguntar
    ...
    console.log(summaryBox(chalk, 'Plano de construção (paralelo)', [
      ...
      ['Agentes', `${agentName} · ${streams.length} agentes, todos em paralelo`],
