...
function launchAgent(chalk, agentCmd, kickoff, label) {
  console.log(chalk.hex('#f5c518')(`\n  Lumen: ${label} — abrindo ${agentCmd}...\n`));
  const res = spawnSync(agentCmd, [kickoff], { stdio: 'inherit', cwd: process.cwd() });
  return res.status ?? 0;
}
...
export default async function build(args) {
  ...
  const scope = what === 'all' ? 'CONSTRUA O SISTEMA INTEIRO a partir de _lumen_docs/' : `construa esta feature: "${desc}"`;
  const kickoff =
  ...
  process.exit(launchAgent(chalk, agentCmd, kickoff, what === 'all' ? 'construindo o SISTEMA INTEIRO' : `construindo a feature: ${desc}`));
}
