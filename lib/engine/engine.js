...
// Executa o motor repassando stdio. Tenta o binário local; se ausente, npx baixa sob demanda.
export function engineRun(args) {
  if (onPath('compozy')) {
    return spawnSync('compozy', args, { stdio: 'inherit' });
  }
  return spawnSync('npx', ['--yes', '-p', '@compozy/cli', 'compozy', ...args], { stdio: 'inherit' });
}
