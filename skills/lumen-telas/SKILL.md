...
1. `.lumen/state.json` → campo `output_folder` (padrão: `_lumen_docs`).
2. `.lumen/config.toml` → seção `[specs]` (campo `granularity`, `custom_folders`).
3. `.lumen/config.user.toml` → seção `[specs]` se existir, com precedência chave a chave.
4. `.lumen/context/surface.json` → `modules`, `organization_suggestion.features`.
...
Quando a pasta da unit ainda não existe (Redator não rodou), crie-a vazia para hospedar os screenshots. O Redator, ao rodar depois, encontra a pasta e adiciona `requirements.md`, `design.md`, `tasks.md` (EC-05).
...
Informe ao Lumen: telas documentadas (e a unit de cada uma), fluxos mapeados.
