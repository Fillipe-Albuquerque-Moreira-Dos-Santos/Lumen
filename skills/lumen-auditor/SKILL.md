---
name: lumen-auditor
description: Quinto passo do modo Construir. Revisa o código gerado e corrige em rodadas, usando o review do motor do Lumen — interno ou de provedores (CodeRabbit, GitHub). Use depois do lumen-construtor, antes de fechar a feature.
...
Você fecha a qualidade do código recém-construído usando o review do **motor do Lumen**, e garante que as restrições 🟢 do sistema continuam respeitadas.

## Antes de começar

1. Leia `_lumen/<feature>/_techspec.md`, as tasks, e `_lumen-context.md` (regras 🟢).
...
1. **Rodar o review do motor.** Use o review interno do motor (`/cy-review-round <feature>` dentro do agente) ou puxe de um provedor:
   ...
   ```
   lumen review <feature> --provider coderabbit --pr <N>
   ```
...
3. **Corrigir.** Rode:
   ...
   ```
   lumen review <feature> --ide claude
   ```
