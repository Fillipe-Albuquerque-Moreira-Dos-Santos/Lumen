---
name: lumen-ground
description: Conector do loop Lumen. Pega a verdade extraída no modo Documentar (`_lumen_docs/`) e produz um grounding pack que o modo Construir consome, carregando a escala de confiança — regras 🟢 viram restrições que o build não pode quebrar. Também semeia o contrato de regressão. Use logo antes de lumen-techspec, numa feature de projeto que já foi documentado.
...
1. `.compozy/tasks/<feature>/_lumen-context.md` — o pack que o `lumen-techspec` (e o motor) leem.
2. `_lumen/<feature>/regression-watch.md` — o contrato que o `lumen-verify` confere depois do build.
...
- Se `_lumen_docs/` não existe ou está vazio:
  ...
  > Rode o modo Documentar (`/lumen`) primeiro. Sem isso não há verdade para fundamentar — se o projeto é novo (greenfield), pule este skill e vá direto para `lumen-prd`.
...
1. `_lumen_docs/architecture.md` → componentes que a feature toca ou vizinha **+ o stack e os padrões arquiteturais que o sistema usa** (linguagens, frameworks back/front, banco, padrão de camadas, tratamento de erro, convenções). Isso é o que o `lumen-techspec` vai sugerir para a feature se encaixar.
...
- Terminar com: `Próximo: lumen-techspec <feature> — este pack é seu input fundamentado; não re-explore o que já está confirmado aqui.`
...
- Deixe uma seção "Histórico de re-extrações" vazia; o `lumen-verify` preenche depois.

## Passo 4 — Relatar

Diga ao usuário: caminho do `_lumen-context.md` (quantos componentes / regras 🟢 / entidades + stack detectado), caminho do `regression-watch.md` (quantos watch items), e o handoff: `Rode lumen-techspec <feature>. Depois do build, rode lumen-verify para conferir os watch items.`
