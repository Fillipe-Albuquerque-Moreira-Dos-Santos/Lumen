---
name: lumen-revisor
description: Revisa criticamente as especificações geradas pelo lumen-redator — encontra inconsistências, reclassifica confiança e gera perguntas para validação humana. Use na fase de revisão de uma análise de engenharia lumen.
license: MIT
...
1. Leia `.lumen/state.json` — especialmente `user_name`, `answer_mode`, `doc_level`, `output_folder` e `engines`
2. Leia `.lumen/config.toml` (e `config.user.toml` se existir) → seção `[specs]` para descobrir a `granularity` e mapa de units
3. Liste as pastas de unit dentro de `<output_folder>/`. Cada unit é uma subpasta com `requirements.md`, `design.md`, `tasks.md` e opcionais. Leia os 3 arquivos canônicos de cada unit, mais os opcionais presentes (`contracts.md`, `flows.md`, `edge-cases.md`, `decisions.md`, `code-mapping.md`, `questions.md`, `screens.md`)
...
> Salve o resultado em `_lumen_docs/cross-review-result.md`.
...
1. Leia `_lumen_docs/cross-review-result.md`
...
Agrupe todas as perguntas em `_lumen_docs/questions.md`.
...
#### Se `answer_mode = "file"`
Crie `_lumen_docs/questions.md` com todas as perguntas formatadas e diga:
> "[Nome], criei `_lumen_docs/questions.md` com [N] perguntas que precisam da sua validação.
> Preencha o campo **Resposta** de cada uma e me avise quando terminar — basta digitar `lumen`."
...
Após processar todas as respostas (ou se não houver lacunas), gere `_lumen_docs/confidence-report.md` seguindo `references/confidence-report-template.md`.
...
- `_lumen_docs/gaps.md` — lacunas que permaneceram sem resposta (se `detalhado`: categorize por severidade: crítico/moderado/cosmético)
- `_lumen_docs/cross-review-result.md` — apontamentos do Codex (se revisão cruzada realizada)
