---
name: lumen-design
description: Extrai e documenta o sistema de design do projeto — paleta de cores, tipografia, espaçamentos, tokens e componentes a partir de CSS, arquivos de tema e screenshots. Use quando arquivos de estilo ou screenshots de interface estiverem disponíveis.
license: MIT
...
Leia `.lumen/state.json` → campo `output_folder` (padrão: `_lumen_docs`). Use-o como pasta de saída.
...
Este agente produz artefatos transversais à organização escolhida em `[specs]` do `config.toml`. Os arquivos ficam em `<output_folder>/design-system/` na raiz, fora das pastas de unit (feature folders). Não aplicar aqui a estrutura `<unit>/requirements.md|design.md|tasks.md`, ela pertence ao Redator.

Informe ao Lumen: tokens documentados por categoria.
