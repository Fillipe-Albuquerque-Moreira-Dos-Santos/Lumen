...
1. `.lumen/config.toml`, seção `[specs]` (config gerenciado pelo Lumen)
2. `.lumen/config.user.toml`, seção `[specs]` (override manual do usuário)
...
> "Detectei que `.lumen/config.toml` não tem decisão de organização das specs, mas `.lumen/config.user.toml` contém um override em `[specs]`. O override continuará ativo após a sua escolha e pode sobrescrever campos que você decidir agora.
>
...
Atualize `.lumen/config.toml`, seção `[specs]`, com:
...
- **Não mexa em `.lumen/config.user.toml`.** Esse arquivo pertence ao usuário.
...
Não existe flag de CLI dedicada para reconfigurar. O usuário reapresenta o menu removendo manualmente a seção `[specs]` de `.lumen/config.toml` (ou esvaziando `granularity`). Na próxima execução, este passo detecta o estado "não decidido" e roda novamente.

## Idioma das pastas (RF-10)

Os nomes que o Lumen usa para as pastas de feature seguem `doc_language` do `state.json`. Não pergunte idioma neste passo. Em uma instalação `pt-br`, as pastas saem em pt-br; em `en`, em inglês.
